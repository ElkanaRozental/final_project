from app.models import Date, City, Country, TargetType, AttackType, Region, Event, Province
def insert_normalized_message(normalized_message, session):
    with session() as session:
        try:
            date_data = normalized_message['date']
            date = (
                session.query(Date)
                .filter_by(year=date_data['year'], month=date_data['month'], day=date_data['day'])
                .first()
            )
            if not date:
                date = Date(**date_data)
